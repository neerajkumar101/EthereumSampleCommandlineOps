...
var convertToAbi = (contractName, locationOfContractSource, formatToReadIn) => {
    return new Promise( (resolve, reject) => {
        // var fs=require('fs');
        
        var readProm = readContractSourceBuildInitial(fs, locationOfContractSource, formatToReadIn, contractName);

        readProm.then((returnArray) => {
            //to write the compiled abi to the json file
            fileStreamWriteJson(fs, __dirname + '/customCheck/checkAbi.json', JSON.stringify(returnArray['abi']), 'utf8')
            .then((message)=>{
                console.log(message);
            })
            .catch((error)=>{
 ...
 */
var createContractInstance = function(contractItself, owner, bytecode, gas, abi, constructorVar) {
    //constructor variables
    var contractName = 'NewTokenName';
    var contdocumentId = 'newdoc123';
 ...
 * @param {string} format,
 * @param {any} constructorVar, 
 * @param {string} contractName,
 * @param {string} reqOwner 
 * @param {string} reqOwnerPassword 
 */
var deployNewSmartContract = function(contractSource, format,constructorVar, contractName, reqOwner, reqOwnerPassword) {
    return new Promise((resolve, reject)=>{
        var owner = reqOwner; 
        var password = reqOwnerPassword;
        // call a function to covert abi(arbitray binary definition) of contract
        var convertToAbiProm = convertToAbi(contractName, contractSource, format);
        ...
        convertToAbiProm.then( (returnArray) => {
            ...
            unlockProm.then( (unlockFlag) => {
                ...
                gasProm.then( (gas) => {
                    var contractProm = createContractInstance(contractItself, owner, bytecode, gas, abi, constructorVar);
...
var deployExistingSmartContract = (reqOwner, reqOwnerPassword, smartContractAddress) => {
    return new Promise( (resolve, reject) => {
        fs.readFile(__dirname + '/customCheck/checkAbi.json', 'utf8', function(error, abiRetrieved) {
            if (error) {
    ...
    contractInstance)
{
    new Promise(function(resolve, reject){
        ...
        approveTransactionLimit(anySourceAccountAddress,//'0xdae4b37af60d3c2ac781484027c20ce9b322ca8b', 
        ...
        var transferData = contractInstance.transfer.getData(
            anySourceAccountAddress, targetAccountAddress, tokenAmount);
